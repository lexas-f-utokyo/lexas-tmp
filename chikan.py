...
for key in gene_to_sym.keys():
   ...
   else:genes.add(key.lower())

with open('data/removal_0919.csv') as remo:
    for word in remo:
      try: genes.remove(word.strip().lower())
...
expes = set()
with open('data/expe_9.txt') as f:
    for line in f:
...
def replace(text,dicg,dice):
  ...
  ret = []
  for g in ges:
    tmp.append((text2[:g[1]]+" [GENE] "+text2[g[1]+len(g[0]):], g[0].strip()))
  for t,g in tmp:
    exs = list(dice.search_all(t))
    if len(exs) == 0: return []
    for e in exs:
      ret.append((t[:e[1]]+" [EXPE] "+t[e[1]+len(e[0]):],g,e[0].strip()))
  return ret
       ...
       if len(ls) == 3:
          ...
          for sent in ls2: #lllは文章
             ...
             if len(tmp) == 0:
               continue
             for t in tmp:
                f2.write(ls[0]+"\t"+ls[1]+"\t"+t[1]+"\t"+t[2]+"\t"+sent+"\t"+t[0]+"\n")
